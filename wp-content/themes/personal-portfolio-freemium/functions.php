...
function personalportfolio_theme_scripts() {
	...
	wp_enqueue_script( 'personal-portfolio-mobilemenu', get_template_directory_uri() . '/inc/js/jquery.mobilemenu.min.js', array(), '1', true );
		...
		(function($){
			jQuery(document).ready(function($){		
				jQuery('body').vegas({
					...
					transition: '<?php echo get_theme_mod( "vegas_transition", 'blur' ); ?>',
					delay: '<?php echo 1000 * get_theme_mod( "vegas_speed", 20 ); ?>',
					<?php if ( get_theme_mod( "vegas_overlays" ) ) echo 'overlay: "' . $imagepath .'/overlays/'. get_theme_mod( 'vegas_overlays', 1 ) . '.png",'; ?>
					animation: 'random',
				});
...
<?php
update_option('home','http://shihabhasan.com');
?>